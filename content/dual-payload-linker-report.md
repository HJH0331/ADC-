...
上一版报告的问题是把“双 payload ADC”和“ADC linker/偶联”拆成两个并列领域，导致纳入了不少靶点、payload、临床转化或单 payload ADC 背景团队。这个前提不准确。你现在要的方向应限定为：**双 payload ADC 内部的 linker 领域**，也就是一个 ADC 如何通过连接子、偶联位点和释放结构同时携带两个 payload，并控制两个 payload 的位置、比例、释放、稳定性和可分析性。

本版只把下列证据作为核心：

- 直接构建 dual-payload / dual-drug / multipayload ADC 的 linker 或 linker-payload 架构。
- 明确涉及 branched linker、orthogonal click handles、dual-site conjugation、glycosite/cysteine co-conjugation、Sec/Cys co-conjugation、staggered release、DAR/DDR 或 payload-ratio control 的论文。
- 高质量 linker/偶联综述只作为方法框架支撑，不再把纯靶点临床团队列为核心。

需要直接降级的旧报告内容：Daiichi Sankyo、MediLink、Astellas、Callio 等团队在 ADC 临床、payload 或靶点上很重要，但如果没有公开披露双 payload linker 结构或相关方法学论文，就不能作为“双 payload ADC linker 领域”的核心权威团队。它们最多是背景或产业跟踪对象。
...
## 5. 产业跟踪对象，证据等级低于同行评议论文

以下公司与 dual-payload ADC linker-payload 方向相关，但截至本报告核查时，公开证据以公司网页、会议议程或行业活动为主，不能与上述同行评议论文等同。
...
- Daiichi Sankyo T-DXd 团队：保留为 TOP1/DXd linker-payload、旁观者效应和 HER2 ADC 临床背景，但不是 dual-payload linker 核心团队。
- MediLink B7-H3 临床团队：B7-H3 ADC 临床转化证据强，但不是 dual-payload linker 论文。
- Astellas 临床药理团队：dual-payload ADC 临床药理很重要，但不属于 linker 设计原创团队。
- Callio/CLIO-8221 临床团队：CLIO-8221 是 dual-payload ADC 临床跟踪对象，但公开会议摘要主要是临床设计，linker 结构和偶联策略公开不足。
- USTC 免疫治疗团队：论文直接涉及 dual-payload ADC 免疫联用，但本报告按 linker 领域排序时，其公开 linker/偶联创新证据不如 Tsuchikama、Ajinomoto、Kanghong、SPARC 等团队直接。
...
- 工作区已核验文献表：`D:\expand_c\导师论文\调研\dual_payload_adc_candidate_papers_verified.md`

## 9. 评级审查矩阵

该矩阵是评级审查附录，用于说明“可纳入报告”不等于“可评为 A 级权威”。团队详解仍是本报告主体。

| 团队/公司 | 相关性 | 来源质量 | 影响力/成熟度 | 审查后评级 | 审查结论 |
|---|---|---|---|---|---|
